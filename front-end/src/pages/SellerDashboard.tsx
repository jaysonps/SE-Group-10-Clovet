      ...
      <aside className="w-80 bg-white border-r border-gray-100 flex flex-col sticky top-16 h-[calc(100vh-64px)] shadow-2xl z-20">
        <div className="p-10 space-y-12">
          <div className="space-y-4">
            <p className="text-[10px] font-black text-gray-300 uppercase tracking-[0.3em] pl-4">
              Seller Control
            </p>
            <nav className="space-y-2">
              <Link
                ...
                className={cn(
                  ...
                  path === "/seller"
                    ? "bg-authentic text-white shadow-xl"
...
function Overview() {
  ...
  const [stats, setStats] = useState<any>({
    activeProducts: 0,
    ranges: {
      "7D": { revenue: 0, totalSales: 0, chartData: [] },
      "1M": { revenue: 0, totalSales: 0, chartData: [] },
      "3M": { revenue: 0, totalSales: 0, chartData: [] },
      ALL: { revenue: 0, totalSales: 0, chartData: [] },
    },
  });
  ...
  React.useEffect(() => {
    ...
    fetch(`/api/orders?limit=${itemsPerPage}&offset=${offset}`, { headers: authHeaders })
      .then((res) => res.json())
      .then((data) => {
  ...
  const activeRangeStats = stats.ranges?.[range] || {
    revenue: 0,
    totalSales: 0,
    chartData: [],
  };
  const currentRevenue = activeRangeStats.revenue;
  const currentSalesCount = activeRangeStats.totalSales;

  return (
    <div className="space-y-16">
      <div className="flex justify-between items-end">
        <div className="space-y-2">
          <h1 className="text-5xl font-display font-black tracking-tighter uppercase leading-tight">
            Overview
          </h1>
        ...
        <div className="sleek-card p-10 border-none space-y-6 bg-white">
          ...
          <div className="space-y-1">
            <p className="text-4xl font-display font-black tracking-tighter text-black">
              {isLoading ? "..." : `IDR ${(currentRevenue / 1000).toFixed(0)}K`}
            </p>
      ...
      <div className="sleek-card p-12 border-none space-y-10 bg-white">
        ...
        <div className="h-[450px]">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={activeRangeStats.chartData}>
              <CartesianGrid
                strokeDasharray="8 8"
                vertical={false}
                stroke="#f0f0f0"
      ...
      <div className="space-y-8 pb-32">
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-gray-100">
                <th className="px-10 py-8 text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">
                  Transaction ID
                </th>
                <th className="px-10 py-8 text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">
                  Product
            ...
            <tbody className="divide-y divide-gray-50">
              {recentOrders.length > 0 ? (
                recentOrders.map((order) => (
                  ...
                  >
                    <td className="px-10 py-8 font-mono text-[10px] font-black tracking-widest text-black">
                      ORD-{order.id}
                    </td>
                    <td className="px-10 py-8 text-xs font-black uppercase tracking-tight">
                      {order.product_name}
                    </td>
                    <td className="px-10 py-8 text-xs font-black">
                      IDR {Number(order.total_amount).toLocaleString()}
                    </td>
                    <td className="px-10 py-8">
                      <span
                        className={cn(
                          "px-4 py-1.5 text-[9px] font-black rounded-full uppercase tracking-widest text-white shadow-sm",
                ...
                <tr>
                  <td
                    colSpan={4}
                    className="px-10 py-20 text-center text-[10px] font-black uppercase tracking-widest text-gray-300"
                  >
        ...
        {totalOrders > itemsPerPage && (
          <div className="flex justify-center items-center space-x-4 pt-8">
            ...
            <button
              disabled={
                currentPage >= Math.ceil(totalOrders / itemsPerPage) - 1
              }
              onClick={() => setCurrentPage((prev) => prev + 1)}
              className="px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest bg-gray-50 border border-gray-100 hover:border-black disabled:opacity-20 transition-all"
            >
...
function ProductsList() {
  ...
  const [editPredictedCategories, setEditPredictedCategories] = useState<
    string[]
  >([]);
  ...
  const handleAnalyze = async (
    ...
    if (!name || !description) {
      setToast({
        type: "warning",
        text: "Please provide both product title and description for AI analysis.",
      });
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.details || errorData.error || "Failed to analyze",
        );
      }
      ...
      if (data.categories && data.categories.length > 0) {
        ...
        setToast({
          type: "error",
          text: "AI could not suggest categories for this product. Please check your title/description.",
        });
      }
    } catch (error) {
  ...
  const handleUpdate = async (updatedData: any) => {
    try {
      ...
      if (response.ok) {
        ...
        setToast({ type: "success", text: "Product details successfully updated log" });
      } else {
        setToast({ type: "error", text: "Failed to update product details" });
      }
    } catch (error) {
      console.error("Update failed:", error);
      setToast({ type: "error", text: "Connection error. Update failed." });
    }
  };
    ...
    try {
      const response = await fetch("/api/products", {
        ...
        body: JSON.stringify({
          ...
          image:
            newProductImage ||
            "https://images.unsplash.com/photo-1521572267360-ee0c2909d518?w=400&q=90",
      ...
      if (response.ok) {
        ...
        setShowAdd(false);
        setToast({ type: "success", text: "New product successfully listed in inventory" });
      } else {
        setToast({ type: "error", text: "Failed to publish new product list" });
      }
    } catch (error) {
      console.error("Publish failed:", error);
      setToast({ type: "error", text: "Connection error. Publish failed." });
    }
  };
    ...
    <div className="space-y-16">
      <div className="flex justify-between items-center">
        <div className="space-y-2">
          ...
          <p className="sleek-label opacity-40">
            Manage and catalog your exclusive products
          </p>
        </div>
        <button
      ...
      {showAdd ? (
        <motion.div
          ...
          className="sleek-card p-12 border-none space-y-12 bg-white shadow-2xl"
        >
          <div className="flex justify-between items-center border-b border-gray-100 pb-8">
            ...
            <button
              ...
              className="p-4 bg-gray-50 text-gray-400 hover:text-black rounded-2xl transition-all"
            >
              ...
              <div className="space-y-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                  <div className="space-y-4">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleNewOriginalPriceChange(e.target.value)
                      }
                      placeholder="0"
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex gap-4">
                      <input
                        ...
                        onChange={(e) =>
                          handleNewDiscountChange(e.target.value)
                        }
                        placeholder="0"
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6">
                    <button
                      onClick={() =>
                        handleAnalyze(newProductName, newProductDesc)
                      }
                    ...
                    <div className="flex-1 flex flex-wrap gap-2 items-center">
                      ...
                      {newProductAnalyzed ? (
                        predictedCategories.map((cat) => (
                          <button
                            key={cat}
                            className="px-4 py-2 bg-white border border-gray-100 rounded-xl text-[9px] font-black uppercase tracking-widest shadow-sm hover:border-black transition-all"
                          >
                            {cat}
                          </button>
                        ))
                ...
                <div className="space-y-6">
                  <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">
                    Inventory Supply by Size
                  </p>
                  <div className="grid grid-cols-5 gap-4">
                    {["XS", "S", "M", "L", "XL"].map((size) => (
                      <div
                        key={size}
                        className="p-4 bg-gray-50 rounded-[1.5rem] space-y-3 border border-gray-100 flex flex-col items-center"
                      >
                        <span className="text-[10px] font-black text-gray-300 uppercase tracking-widest">
                          {size}
                        </span>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-gray-100">
                  ...
                  <th className="px-10 py-8 text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] text-right">
                    Actions
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-50">
                {products.map((product) => (
                  ...
                  >
                    ...
                    <td className="px-10 py-8 text-xs font-black uppercase tracking-widest text-gray-400">
                      {product.category}
                    </td>
                    <td className="px-10 py-8 text-xs font-black uppercase tracking-widest text-gray-400">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                      <div className="space-y-4">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            handleEditOriginalPriceChange(e.target.value)
                          }
                          placeholder="0"
                          className="w-full bg-gray-50 border border-gray-100 rounded-[1.5rem] px-8 py-4 outline-none focus:ring-2 focus:ring-black font-black text-sm tracking-tight"
                        />
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="flex gap-4">
                          <input
                            ...
                            onChange={(e) =>
                              handleEditDiscountChange(e.target.value)
                            }
                            placeholder="0"
                            className="w-20 bg-gray-50 border border-gray-100 rounded-[1.5rem] px-4 py-4 outline-none focus:ring-2 focus:ring-black font-black text-sm tracking-tight text-center"
                          />
                          <div className="flex-1 flex gap-2">
                            {[10, 20, 30, 50].map((pct) => (
                              <button
                                ...
                                type="button"
                                onClick={() =>
                                  handleEditDiscountChange(String(pct))
                                }
                                className={cn(
                                  "flex-1 rounded-[1rem] text-[10px] font-bold transition-all border border-gray-100",
                                  Number(editProductDiscount) === pct
                                    ? "bg-black text-white"
                                    : "bg-white text-gray-500 hover:border-black",
                                )}
                              >
                      ...
                      <div className="space-y-4">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            handleEditSalePriceChange(e.target.value)
                          }
                          placeholder="0"
                          className="w-full bg-gray-50 border border-gray-100 rounded-[1.5rem] px-8 py-4 outline-none focus:ring-2 focus:ring-black font-black text-sm tracking-tight"
                        />
        ...
        {deleteConfirmId && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 40 }}
              className="relative w-full max-w-md sleek-card border-none p-12 shadow-2xl space-y-8 bg-zinc-900 border border-zinc-800 text-white"
            >
              <div className="flex justify-between items-center pb-6 border-b border-zinc-800">
                <h3 className="text-2.5xl font-display font-black tracking-tight uppercase text-white">Confirm Deletion</h3>
                <button
                  onClick={() => setDeleteConfirmId(null)}
                ...
                <button
                  onClick={async () => {
                    ...
                    try {
                      ...
                      if (response.ok) {
                        setToast({ type: "success", text: "Product successfully deleted from inventory" });
                        await fetchProducts();
                      } else {
                        setToast({ type: "error", text: "Failed to delete product from database" });
                      }
                    } catch (error) {
                      console.error("Delete failed:", error);
                      setToast({ type: "error", text: "Connection error. Failed to delete product." });
                    }
                  }}
                  className="flex-1 py-4 bg-rose-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-rose-700 transition-all shadow-lg"
                >
                  Delete Product
                </button>
                <button
                  onClick={() => setDeleteConfirmId(null)}
                  className="flex-1 py-4 bg-zinc-800 text-zinc-400 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:border-white transition-all border border-zinc-700"
                >
                  Cancel
                </button>
              </div>
        ...
        {toast && (
          ...
          >
            <div className="flex items-center space-x-3">
              <AlertCircle size={18} className={toast.type === "error" ? "text-rose-500" : "text-amber-500"} />
...
function OrdersManagement() {
  ...
  React.useEffect(() => {
    fetchOrders();
  }, [currentPage, activeStatus, orderSearchQuery]);
  ...
  const statuses = [
    { label: "Submitted", value: "Submitted", disabled: false },
    { label: "Processing", value: "Processing", disabled: false },
    { label: "In Verification", value: "In Verification", disabled: false },
    { label: "Shipped", value: "Shipped", disabled: false },
    ...
    <div className="space-y-16">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8">
        <div className="space-y-2">
          ...
          <p className="sleek-label opacity-40">
            Transaction ledger and fulfillment logistics
          </p>
        </div>
        <div className="flex flex-col sm:flex-row gap-6 w-full lg:w-auto">
          <div className="relative flex-1 sm:flex-none">
            ...
            <input
              ...
              className="bg-white border border-gray-100 rounded-2xl py-4 pl-14 pr-8 text-[10px] font-black uppercase tracking-widest outline-none shadow-xl focus:ring-2 focus:ring-black placeholder:text-gray-200 w-full"
            />
          </div>
            ...
            {displayOrders.length > 0 ? (
              displayOrders.map((order) => (
                <tr
                  ...
                  className="hover:bg-gray-50/50 transition-all cursor-pointer"
                >
                  <td className="px-10 py-8 font-black font-mono text-[10px] tracking-widest text-black">
                    {order.id}
                  </td>
                  <td className="px-10 py-8 text-sm font-black uppercase tracking-tight">
                    {order.prod}
                  </td>
                  <td className="px-10 py-8 text-xs font-bold text-gray-400 uppercase tracking-widest">
                    {order.cust}
                  </td>
                  <td className="px-10 py-8 text-[10px] font-black uppercase tracking-widest text-gray-300">
                    {order.date}
                  </td>
                  <td className="px-10 py-8">
                    <span
                      className={cn(
                        ...
                        order.status === "Completed" || order.status === "COMPLETED"
                          ? "bg-green-50 text-green-600 border border-emerald-100"
                          : order.status === "Rejected" || order.status === "REJECTED"
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-12">
                <div className="space-y-2">
                  ...
                  <p className="text-xl font-display font-black uppercase">
                    {selectedOrder.prod}
                  </p>
                </div>
                <div className="space-y-2">
                ...
                <div className="space-y-2">
                  ...
                  <p
                    className={cn(
                      ...
                      selectedOrder.status === "Rejected" || selectedOrder.status === "REJECTED"
                        ? "text-red-500"
              ...
              {selectedOrder.status === "Processing" && (
                <div className="p-8 bg-zinc-50 rounded-[2rem] space-y-4 border border-zinc-100">
                  ...
                  <p className="text-xs text-zinc-500">
                    Please print the shipping label, send the product physically
                    to the Clovet Verification Center, and save the Courier
                    Tracking Code (Resi Pengiriman) below:
                  </p>
                      ...
                      onClick={async () => {
                        const rcpt = (
                          document.getElementById(
                            "seller-input-tracking",
                          ) as HTMLInputElement
                        )?.value;
                        if (!rcpt) {
                          setToast({ type: "warning", text: "Please enter a tracking number/resi" });
                          return;
                        }
                        try {
                          const response = await fetch(
                            `/api/orders/${selectedOrder.rawId}/ship`,
                            {
                              method: "PATCH",
                              headers: authHeaders,
                          ...
                          if (response.ok) {
                            ...
                            fetchOrders();
                            setToast({ type: "success", text: "Order status successfully updated to SHIPPED" });
                          } else {
                            setToast({ type: "error", text: "Failed to update order to shipped" });
                          }
                        } catch (err) {
                          console.error(err);
                          setToast({ type: "error", text: "Connection error. Failed to update shipment." });
                        }
                      }}
              ...
              {selectedOrder.trackingCode && (
                <div className="p-8 bg-zinc-50 rounded-[2rem] space-y-2 border border-zinc-100">
                  ...
                  <p className="text-sm font-mono font-black uppercase text-black">
                    {selectedOrder.trackingCode}
                  </p>
                </div>
              )}
              ...
              <div className="space-y-8">
                ...
                {selectedOrder.status === "Rejected" || selectedOrder.status === "REJECTED" ? (
                  <div className="p-10 border border-red-100 rounded-[2.5rem] bg-red-50/50 space-y-6 shadow-sm">
                    <div className="flex justify-between items-start">
                      <div className="space-y-4">
                        <div className="flex items-center space-x-4">
                          <span className="text-lg font-display font-black uppercase tracking-tight text-red-600">
                            Verifier Inspection Detail
                          </span>
                          <span className="px-3 py-1 bg-red-100 text-red-600 text-[8px] font-black uppercase tracking-widest rounded-md">
                            Rejected / Counterfeit
                          </span>
                        </div>
                      </div>
                    </div>
                    <p className="text-sm font-bold leading-relaxed italic text-red-900 bg-white border border-red-100/80 p-6 rounded-2xl">
                      "{selectedOrder.verification_notes || 'No descriptive evidence documented by verifier.'}"
                    </p>
                    {selectedOrder.verification_evidence_image && (
                      <div className="aspect-video w-full rounded-2xl overflow-hidden border border-red-100 shadow-inner">
                        <img 
                          src={selectedOrder.verification_evidence_image} 
                          alt="Verifier Evidence" 
                          className="w-full h-full object-cover"
                  ...
                  <div className="p-10 border border-gray-100 rounded-[2.5rem] bg-white space-y-8 shadow-sm">
                    <div className="flex justify-between items-start">
                      <div className="space-y-4">
                        <div className="flex items-center space-x-4">
                          <span className="text-lg font-display font-black uppercase tracking-tight">
                            {selectedOrder.cust}
                          </span>
                          <span className="px-3 py-1 bg-emerald-50 text-emerald-500 text-[8px] font-black uppercase tracking-widest rounded-md">
                            Verified Purchase
                          </span>
                        </div>
                      ...
                      <span className="text-[10px] font-black text-gray-300 uppercase">
                        {selectedOrder.review.date}
                      </span>
                    </div>
                    <p className="text-sm font-bold leading-relaxed italic text-zinc-600">
...
function FinanceSection() {
  ...
  const fetchFinanceData = () => {
    setIsLoading(true);
    fetch("/api/seller/finance", { headers: authHeaders })
      .then((res) => res.json())
      .then((d) => {
        setData(d);
        setIsLoading(false);
  ...
  const handlePrepareExtract = async () => {
    ...
    // REQ-F6-2: Request OTP from backend (simulated SMS)
    try {
      const res = await fetch("/api/seller/otp/request", {
        method: "POST",
        headers: authHeaders,
      });
      const otpData = await res.json();
      setGeneratedOtp(otpData.otp_simulation); // store for display only (simulation)
      setShowOtp(true);
      setOtpValue("");
      setMessage({
        type: "success",
        text: `🔑 CLOVET SECURE: Your 6-digit extraction code is ${otpData.otp_simulation} (simulation — in production this arrives via SMS)`,
      });
    } catch (e) {
  ...
  const handleExtract = async () => {
    ...
    try {
      const response = await fetch("/api/seller/extract", {
        method: "POST",
        headers: authHeaders,
        body: JSON.stringify({ ...extractionForm, otp_code: otpValue }),
      });
      if (response.ok) {
    ...
    <div className="space-y-16">
      <div className="flex justify-between items-center">
        <div className="space-y-2">
          ...
          <p className="sleek-label opacity-40">
            Wallet management and revenue extraction
          </p>
        </div>
      </div>
        ...
        <div className="sleek-card rounded-[3rem] p-12 border-none space-y-8 shadow-2xl bg-white">
          ...
          <div className="space-y-4">
            ...
            <p className="text-xs text-gray-400 font-bold leading-relaxed uppercase tracking-tight">
              Escrow holding active. Funds will unlock post-verification
              completion.
            </p>
          </div>
        </div>
        ...
        <div className="space-y-6">
          {data.ledger.length > 0 ? (
            data.ledger.map((item: any) => (
              <div
                key={item.id}
                className="flex items-center justify-between p-8 border border-gray-50 rounded-[2rem] hover:bg-gray-50/50 transition-all group"
              >
                <div className="flex items-center space-x-6">
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="p-10 bg-zinc-900 rounded-[2.5rem] text-white shadow-2xl">
                <p className="text-[10px] uppercase font-black tracking-[0.3em] text-white/30 mb-2">
                  Liquid Balance
                </p>
              ...
              {showOtp ? (
                <div className="space-y-8">
                  <div className="space-y-2">
                    ...
                    <p className="text-[10px] text-gray-400 font-bold uppercase tracking-tight leading-relaxed mb-4">
                      Enter the 6-digit confirmation code sent to your registered contact.
                    </p>
                    <input
                      type="text"
                      placeholder="• • • • • •"
                      maxLength={6}
                      className="sleek-input text-center text-2xl tracking-[1em] font-mono"
                      value={otpValue}
                  ...
                  <div className="p-6 bg-gray-50 rounded-[2rem] flex items-start space-x-4 border border-gray-100 border-dashed">
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-[9px] text-gray-400 font-bold uppercase tracking-tight leading-relaxed">
                        Identity verification required for all large-scale
                        extractions. Transaction validation in progress.
                      </p>
                    </div>
                  </div>
