...
export default function Header() {
  ...
  const isDashboard = location.pathname.startsWith('/seller') || location.pathname.startsWith('/verifier') || userRole !== 'customer';
      ...
      <div className="container mx-auto px-4 lg:px-8">
        ...
        <div className="flex h-16 items-center justify-between gap-8 py-10">
          ...
          <div className="flex items-center space-x-6 text-sm font-semibold">
            {user ? (
              <Link to="/profile" className="w-12 h-12 rounded-full overflow-hidden border-2 border-white shadow-lg hover:scale-105 transition-transform bg-gray-100 flex items-center justify-center">
                {user.avatar ? (
        ...
        {/* Bottom Row: Categories - Hidden on Dashboard */}
        {!isDashboard && (
